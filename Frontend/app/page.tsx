...
import { LogOut, MessageCircle, Moon, Sun, Wifi, WifiOff } from 'lucide-react'
...
function ChatApp() {
  ...
  useEffect(() => {
    ...
    ;(async () => { setError(null); setIsLoadingConversations(true); try { const user = await chatApi.getCurrentUser(token); const raw = await chatApi.getConversations(token); const normalized = raw.map((conv) => normalizeConversation(conv)); setCurrentUser(user); setConversations(normalized); setUnreadCounts(Object.fromEntries(normalized.map((conv) => [conv.id, 0]))); setSocketStatus('connecting'); await chatSocket.connect(token); chatSocket.on('connect', handleConnect); chatSocket.on('disconnect', handleDisconnect); chatSocket.onNewMessage(handleNewMessage); setSocketStatus('connected') } catch { setSocketStatus('error'); setError('Could not load your conversations') } finally { setIsLoadingConversations(false) } })()
  ...
  const startConversation = async (user: User) => { if (!token || !currentUser) return; try { const created = await chatApi.createConversation(user.id, token); const conversation = normalizeConversation(created); const next = [conversation, ...conversations.filter((item) => item.id !== conversation.id)]; setConversations(next); setActiveConversationId(conversation.id); setShowDiscovery(false); setMobileView('chat') } catch (cause) { setError(cause instanceof Error ? cause.message : 'Could not start conversation') } }

  useEffect(() => { if (!activeConversationId || !token) return; console.log('[CHAT] Joining conversation:', activeConversationId); chatSocket.joinRoom(activeConversationId); let mounted = true; setError(null); setIsLoadingMessages(true); chatApi.getMessages(activeConversationId, token).then((msgs) => mounted && setMessages(activeConversationId, msgs)).catch(() => mounted && setError('Could not load messages')).finally(() => mounted && setIsLoadingMessages(false)); return () => { mounted = false; chatSocket.leaveRoom(activeConversationId) } }, [activeConversationId, setMessages, token, setError, setIsLoadingMessages])
